    ...
    <div slot="dialog-area" class="article-dialog-input">
      ...
      >
        ...
        >
          <img
            v-if="postForm.img"
            :src="postForm.img"
            class="article-home-img"
          />
          <el-form-item :label="$t('postForm.title.label')" prop="title">
            <el-input
              v-model="postForm.title"
              :placeholder="$t('postForm.title.placeholder')"
              :disabled="type === 'edit'"
              clearable
            ></el-input>
          </el-form-item>
          <el-form-item :label="$t('postForm.path.label')" prop="path">
            <el-input
              ...
              :placeholder="$t('postForm.path.placeholder')"
              :disabled="type === 'edit'"
              clearable
            ></el-input>
          </el-form-item>
          <el-form-item
            :label="$t('postForm.description.label')"
            prop="description"
          >
            <el-input
              type="textarea"
              v-model="postForm.description"
              :placeholder="$t('postForm.description.placeholder')"
              clearable
            ></el-input>
          </el-form-item>
          <el-form-item :label="$t('postForm.author.label')" prop="author">
            <el-input
              ...
              :placeholder="$t('postForm.author.placeholder')"
              clearable
            ></el-input>
          </el-form-item>
          <el-form-item
            :label="$t('postForm.categories.label')"
            prop="categories"
          >
            <el-select
              v-model="postForm.categories"
              :placeholder="$t('postForm.categories.placeholder')"
              style="width: 100%;"
              multiple
              filterable
              allow-create
          ...
          <el-form-item :label="$t('postForm.tags.label')" prop="tags">
            <el-select
              ...
              :placeholder="$t('postForm.tags.placeholder')"
              style="width: 100%;"
              multiple
              filterable
              allow-create
          ...
          <el-form-item :label="$t('postForm.img.label')">
            <el-input
              ...
              :placeholder="$t('postForm.img.placeholder')"
              clearable
            />
          </el-form-item>
          <el-form :inline="true">
      ...
      >
        ...
        >
          <el-table-column
            prop="title"
            :label="$t('postForm.frontmatters.key')"
            width="100"
          >
          ...
          <el-table-column
            prop="value"
            :label="$t('postForm.frontmatters.value')"
          >
            <template scope="scope">
          ...
          <el-table-column width="80" align="center">
            <template slot="header">
              <el-button
                size="mini"
                type="primary"
                @click="frontMatters = [...frontMatters, {}]"
                >{{ $t('postForm.frontmatters.add') }}</el-button
              >
            </template>
            <template slot-scope="scope">
              <el-button
                size="mini"
                type="danger"
                @click="deleteFrontMatter(scope)"
                >{{ $t('postForm.frontmatters.del') }}</el-button
              >
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  updated() {
    ...
    if (this.type === 'preview' && this.$refs && this.$refs.mdEditor) {
      let targetEl = this.$refs.mdEditor.$el.querySelector(
  ...
  methods: {
    ...
    // a标签在浏览器中打开
    renderLink() {
      let markdownBody = document.querySelector(
        '.markdown-body.custom-markdown-editor'
      );
    ...
    deletePost() {
      ...
      this.$confirm(
        this.$t('deleteArticleConfirmMsg'),
        this.$t('confirmTips'),
        {
    ...
    localviewPost() {
      electron.shell.openExternal(
        `http://localhost:4000/${this.post.canonical_path}`
      );
    },
    handleDialogConfirm() {
    ...
    async submitForm() {
      ...
      try {
        ...
        let doPublish =
          this.type === 'edit' && this.draft && submitForm.layout === 'post';
        if (doPublish) {
        ...
        setTimeout(() => {
          // 选中当前
          let filterCurrent = this.allPost.filter(
            v => v.title === submitForm.title
          )[0];
          if (filterCurrent) {
